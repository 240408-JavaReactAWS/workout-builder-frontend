...
function WorkoutPlan() {
    ...
    useEffect(() => {
        let asyncFunc = async () => {
            try {
                let response = await axios.get(`http://ec2-34-224-100-40.compute-1.amazonaws.com/plans/${id}`, { 
    ...
    let deletePlan = async () => {
        try {
            let response = await axios.delete(`http://ec2-34-224-100-40.compute-1.amazonaws.com/plans/${id}`, { 
                withCredentials: true , headers: { 'Content-Type': 'application/json', 'username': localStorage.getItem("username")}
            });
